...
const buscarCorreo = () => {
    form.get(route('correo-globales.index', { search: searchQuery.value }), {
        preserveScroll: true,
    });
};

const crearCorreo = () => {
    form.post(route("correo-globales.store"), {
        preserveScroll: true,
        onSuccess: () => {
            swalWithTailwind.fire({
                title: "Correo global creado correctamente",
                icon: "success",
            });
            closeModal();
        },
...
const eliminarCorreo = (id) => {
    swalWithTailwind.fire({
        title: '¿Seguro que deseas eliminar este correo global?',
        icon: 'question',
        ...
        if (result.isConfirmed) {
            form.delete(route("correo-globales.destroy", id), {
                onSuccess: () => {
                    // Acción después de la eliminación exitosa
                    swalWithTailwind.fire({
                        title: 'Correo global eliminado correctamente',
                        icon: 'success',
                    });
                },
                onError: () => {
